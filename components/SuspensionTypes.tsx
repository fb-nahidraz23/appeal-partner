      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className={`text-center mb-12 sm:mb-16 lg:mb-20 transition-opacity duration-1200 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <h2 className="text-3xl sm:text-5xl lg:text-7xl font-light text-white mb-6 sm:mb-8 tracking-tight leading-[0.9] px-4">
            Suspension Types <span className="font-medium bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">We Handle</span>
          </h2>
          <p className="text-lg sm:text-xl text-blue-200/80 max-w-3xl mx-auto font-light px-4">
            From account-level suspensions to individual listing issues, we&apos;ve successfully resolved every type of marketplace suspension.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
          {suspensionTypes.map((type, index) => (
            ...
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-br ${type.gradient} opacity-0 group-hover:opacity-5 rounded-xl sm:rounded-2xl transition-opacity duration-500`}></div>
              
              <div className="relative z-10">
                {/* Icon */}
                <div className={`w-12 sm:w-14 h-12 sm:h-14 bg-gradient-to-br ${type.gradient} rounded-lg sm:rounded-xl flex items-center justify-center mb-4 sm:mb-5 shadow-lg transition-all duration-500 ${hoveredIndex === index ? 'scale-110 rotate-6' : ''}`}>
                  <i className={`${type.icon} text-lg sm:text-xl text-white`}></i>
                </div>
                ...
                <div className="mt-4 sm:mt-5 flex items-center">
                  ...
                  <span className="text-xs text-emerald-300 font-medium">Expert Resolution</span>
                </div>
              </div>

              {/* Hover effect border */}
              <div className={`absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-br ${type.gradient} opacity-0 group-hover:opacity-20 blur-xl transition-opacity duration-500 -z-10`}></div>
            </div>
          ))}
        </div>

        {/* Bottom CTA */}
        <div className={`text-center mt-12 sm:mt-16 lg:mt-20 transition-opacity duration-1200 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-10 max-w-3xl mx-auto">
            ...
            <p className="text-sm sm:text-base text-blue-200/80 mb-5 sm:mb-6">
              We handle all types of Amazon and Walmart suspensions. Contact us for a free consultation.
            </p>
            <a 
              href="tel:+12137579704"
